    ...
    <div className="h-full flex flex-col">
      <div className="p-4 sm:p-6 border-b border-border">
        <p className="text-xs text-muted-foreground mb-1">
          {transcript.recorded_at
            ? new Date(transcript.recorded_at).toLocaleDateString("en-AU", {
                year: "numeric",
                month: "long",
                day: "numeric",
              })
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 pb-20 md:pb-6">
        {transcript.raw_text_ar ? (
          ...
          <p className="text-sm text-muted-foreground">No transcript text yet.</p>
        )}
...
export function TranscriptsSplitView({ transcripts, lang, selectedId }: Props) {
  ...
  return (
    <div className="flex flex-col md:grid md:grid-cols-[320px_1fr] h-full overflow-hidden">
      {/* Left column */}
      <div
        className={cn(
      ...
      >
        ...
        <div className="p-4 border-b border-border">
          <div className="flex items-center justify-between mb-3">
            <h1 className="text-base font-semibold text-foreground">Transcripts</h1>
            <Link
          ...
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {search ? "No results" : "No transcripts yet"}
              </p>
              {!search && (
                <Link
                  href="/transcripts/new"
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 mb-0.5">
                          <p className="text-sm font-medium text-foreground truncate">
                            {t.recorded_at
                              ? new Date(t.recorded_at).toLocaleDateString("en-AU")
                              : "Unknown date"}
                          </p>
                          {t.proposal_status && (
                            <Badge
